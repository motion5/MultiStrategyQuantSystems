...
def get_sp500_instruments():  # -> List[]
...
def get_symbol_history(symbol: str) -> pd.DataFrame:
    return yf.Ticker(symbol).history(period="10y")  # type: ignore
...
def get_sp500_df():
    ...
    for inst in instruments:
        ...
        # [motion5] used in tutorial, chat gpt suggested avoid lambda for
        #   column renaming,using list comprehension might be faster

        # columns = list(map(lambda x: "{} {}".format(inst, x), inst_df.columns))
        columns = ["{} {}".format(inst, col) for col in inst_df.columns]

        # [motion5] used in tutorial, but producing warning about highly
        #   fragmented DataFrame
        #   df[columns] = inst_df
        inst_df.columns = columns
...
def extend_dataframe(traded, df):
    ...
    # fill missing data first by forward filling data,
    # such that [] [] [] a b c [] [] [] becomes [] [] [] a b c c c c
    """ Used in tutorial but now deprecated"""
    # historical_data.fillna(method="ffill", inplace=True)
    historical_data.ffill()
    # fill missing data by backward filling data,
    ...
    for inst in traded:
        ...
        # test if stock is actively trading by using rough measure
        # of non-zero price change from previous time step
...
"""
Note there are multiple ways to fill missing data, depending on you 
...
The choices differ for your requirments, for instance, in backtesting 
you might favour (1), while in training neural models you might favour (4)

Note that the data cycle can be very complicated, with entire reearch 
teamsdedicated to obtaining, processing and extracting signals from 
structure/unstructured data.
What we show today barely scratches the surface of the entire process, 
since we are dealing with well behaved data that is structued and 
already cleand for us by Yahoo Finance API
"""
